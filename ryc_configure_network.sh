...
# Usage: ./network-config.sh [--mode dhcp|static] [--ip IP_ADDRESS] [--netmask NETMASK] [--gateway GATEWAY] [--dns DNS_SERVERS] [--hostname HOSTNAME] [--interface INTERFACE] [--test-ip TEST_IP]
...
usage() {
  echo "Usage: $0 [--mode dhcp|static] [--ip IP_ADDRESS] [--netmask NETMASK] [--gateway GATEWAY] [--dns DNS_SERVERS] [--hostname HOSTNAME] [--interface INTERFACE] [--test-ip TEST_IP]"
  exit 1
}
...
if [ -z "$MODE" ]; then
  echo "Do you want to configure networking for DHCP or Static IP? [dhcp/static]"
...
EOF
  ...
  log "Using IP address: $IP_ADDRESS/$NETMASK"
  log "Gateway: $GATEWAY"
